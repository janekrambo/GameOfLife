...
char * bmpDIBHeader;
long DIB_head_size = 0.0;
int remain = 0;			//liczba pustych bitów w wierszu pliku bmp
...
int ReadFile(char* inputFile, int &arrayDimension){
	...
	else
	{
        ...
        printf("Rozmiar nagłówka DIB = %ld\n", DIB_head_size);
        ...
        int offset; //początkowy adres bitow w tabeli pikseli
        ...
        int biWidth;
        fread(&biWidth, 4, 1, fptr);
        int biHeight;
         fread(&biHeight, 4, 1, fptr);

            printf("binary width: %d \n", biWidth);
